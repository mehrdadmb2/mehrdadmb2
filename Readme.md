<div align="center">

<img src="https://capsule-render.vercel.app/api?type=venom&color=gradient&customColorList=12,20,23&height=280&section=header&text=MEHRDAD&fontSize=88&fontColor=B44FE8&animation=fadeIn&fontAlignY=36&desc=Software%20Developer%20%7C%20IoT%20Builder%20%7C%20Embedded%20Systems%20Enthusiast&descAlignY=60&descSize=17&descColor=E0AAFF&stroke=B44FE8&strokeWidth=1.4"/>

<br/>

[![Typing SVG](https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=700&size=24&duration=3000&pause=700&color=B44FE8&center=true&vCenter=true&random=false&width=900&height=50&lines=%24+whoami+%E2%86%92+Mehrdad+%7C+Computer+Engineering;%24+cat+skills.log+%7C+grep+%22IoT+%7C+Software+%7C+3D%22;%24+python+build_something_useful.py;%24+git+commit+-m+%22Turn+ideas+into+reality%22;%24+sudo+apt+upgrade+knowledge;%24+status+%E2%86%92+Always+learning%2C+always+building)](https://github.com/mehrdadmb2)

<br/>

[![GitHub](https://img.shields.io/badge/GitHub-mehrdadmb2-181717?style=for-the-badge&logo=github)](https://github.com/mehrdadmb2)
[![Portfolio](https://img.shields.io/badge/Portfolio-Visit_Website-B44FE8?style=for-the-badge&logo=vercel&logoColor=white)](https://mehrdadmb2.github.io/mehrdad-dev/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/mehrdad-mb-658520232)
[![Instagram](https://img.shields.io/badge/Instagram-Follow-E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://www.instagram.com/_._.m._.b)
[![Discord](https://img.shields.io/badge/Discord-0mehrdad0-5865F2?style=for-the-badge&logo=discord&logoColor=white)](https://discord.gg/0mehrdad0)
[![X](https://img.shields.io/badge/X-@__Mehrdad_-000000?style=for-the-badge&logo=x&logoColor=white)](https://x.com/__Mehrdad_)

</div>

---

## `~/about $ cat profile.json`

```json
{
  "name": "Mehrdad",
  "username": "mehrdadmb2",
  "location": "Shiraz, Iran",
  "field": "Computer Engineering - Software",
  "roles": [
    "Software Developer",
    "IoT Builder",
    "Embedded Systems Enthusiast",
    "3D Designer"
  ],
  "interests": [
    "Internet of Things",
    "Automation",
    "Backend Development",
    "System Programming",
    "3D Design and Visual Effects"
  ],
  "current_status": "Building skills, projects, and a stronger future."
}
```

---

## `~/intro $ ./about-me`

```text
I am a Computer Engineering student and technology enthusiast from Shiraz.

I enjoy building practical projects that connect software with real-world
hardware. Most of my projects involve ESP32, Arduino, sensors, web dashboards,
automation, data logging, networking, and embedded systems.

I also work with Python, C/C++, C#, web technologies, Linux, Blender,
SolidWorks, and other development and design tools.

My main goal is simple:
Learn continuously, build useful products, and turn technical ideas into reality.
```

---

## `~/focus $ ls -la`

| Area | What I Work On |
|---|---|
| **IoT & Embedded Systems** | ESP32, Arduino, sensors, OLED displays, RFID, SD card logging and device communication |
| **Software Development** | Python tools, desktop applications, automation scripts and backend services |
| **Web Development** | Responsive dashboards, REST APIs, GitHub Pages and frontend interfaces |
| **Networking & Linux** | Network troubleshooting, Linux tools, Nginx, Docker and self-hosted services |
| **3D & Visual Design** | Blender, Cinema 4D, SolidWorks, modeling, rendering and visual effects |

---

## `~/stack $ neofetch --skills`

<div align="center">

<img src="https://skillicons.dev/icons?i=python,cpp,c,cs,js,html,css,nodejs,dotnet,arduino,raspberrypi,docker,linux,bash,nginx,sqlite,mysql,git,github,vscode,visualstudio,blender&theme=dark&perline=11" alt="Technology stack"/>

</div>

<br/>

<div align="center">

| Category | Technologies |
|---|---|
| **Languages** | Python, C, C++, C#, JavaScript, HTML, CSS |
| **Embedded & IoT** | ESP32, Arduino, Raspberry Pi, DHT22, OLED, RFID, SPI, I²C |
| **Backend** | .NET, Node.js, REST APIs, Nginx |
| **Databases** | SQLite, MySQL, Microsoft SQL Server |
| **DevOps & Tools** | Git, GitHub, Docker, Linux, Bash, VS Code, Visual Studio |
| **3D & Engineering** | Blender, Cinema 4D, SolidWorks |

</div>

---

## `~/projects $ find ./featured -maxdepth 1`

### 🏠 Smart Home Hybrid IoT

A multi-device smart-home platform built around ESP32 boards, sensors, local storage and web dashboards.

```yaml
features:
  - Environmental monitoring
  - Temperature and humidity history
  - SD card data logging
  - OLED status interface
  - RFID access control
  - Remote door control
  - REST API communication
  - Responsive admin dashboard
```

### 📡 ESP32 Environmental Monitor

A standalone monitoring node that collects sensor readings, displays system information and stores daily logs.

```yaml
hardware:
  - ESP32
  - DHT22
  - OLED display
  - MicroSD card module

software:
  - Wi-Fi connectivity
  - Daily CSV log files
  - Web dashboard
  - Status and history APIs
```

### 🛠️ Python Utility Applications

Desktop and command-line tools for automation, network testing, data processing and system troubleshooting.

```yaml
focus:
  - User-friendly interfaces
  - Reliable error handling
  - Exportable reports
  - Multithreading
  - Portable single-file applications
```

### 🌐 Personal Developer Portfolio

A modern portfolio website for presenting my skills, projects, experience and contact information.

[![Repository](https://img.shields.io/badge/Repository-mehrdad--dev-B44FE8?style=flat-square&logo=github)](https://github.com/mehrdadmb2/mehrdad-dev)
[![Live Website](https://img.shields.io/badge/Live-GitHub_Pages-9D4EDD?style=flat-square&logo=githubpages)](https://mehrdadmb2.github.io/mehrdad-dev/)

---

## `~/learning $ cat roadmap.yml`

```yaml
currently_learning:
  - Advanced ESP32 development
  - IoT architecture and device communication
  - Backend API design
  - Docker and self-hosted automation
  - Machine learning fundamentals
  - Professional 3D workflows

next_targets:
  - Build production-ready IoT systems
  - Publish more open-source projects
  - Improve software architecture skills
  - Create independent technology products
```

---

## `~/stats $ ./github-analytics`

<div align="center">

<!-- ============ METRICS (lowlighter/metrics) ============ -->
<!-- این بخش توسط GitHub Action به‌روز میشه -->
<img src="https://raw.githubusercontent.com/mehrdadmb2/mehrdadmb2/main/metrics.svg" width="100%" alt="Metrics Dashboard"/>

<br/><br/>

<!-- ============ 3D CONTRIBUTION ============ -->
<img src="https://raw.githubusercontent.com/mehrdadmb2/mehrdadmb2/main/profile-3d-contrib/profile-night-rainbow.svg" width="100%" alt="3D Contribution Graph"/>

<br/><br/>

<!-- ============ ACTIVITY GRAPH ============ -->
<img src="https://github-readme-activity-graph.vercel.app/graph?username=mehrdadmb2&bg_color=0d0d0d&color=B44FE8&line=9D4EDD&point=E0AAFF&area=true&area_color=3C096C&hide_border=true&custom_title=Contribution%20Activity&title_color=B44FE8&radius=6" width="100%" alt="Contribution activity graph"/>

<br/>

<!-- ============ STATS & LANGUAGES ============ -->
<img src="https://github-readme-stats.vercel.app/api?username=mehrdadmb2&theme=radical&hide_border=true&include_all_commits=true&count_private=true&show_icons=true&rank_icon=percentile&icon_color=B44FE8&title_color=B44FE8&text_color=E0AAFF&bg_color=0d0d0d" height="180" alt="GitHub statistics"/>

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=mehrdadmb2&theme=radical&hide_border=true&layout=donut&langs_count=8&title_color=B44FE8&text_color=E0AAFF&bg_color=0d0d0d" height="180" alt="Most used languages"/>

<br/>

<!-- ============ STREAK ============ -->
<img src="https://streak-stats.demolab.com?user=mehrdadmb2&theme=radical&hide_border=true&background=0D0D0D&ring=B44FE8&fire=9D4EDD&currStreakLabel=E0AAFF&sideLabels=B44FE8&dates=7C3AED" width="70%" alt="GitHub contribution streak"/>

<br/><br/>

<!-- ============ WAKATIME (اختیاری) ============ -->
<!-- اگر از WakaTime استفاده می‌کنید، این بخش رو فعال کنید -->
<!-- 
[![Wakatime](https://github-readme-stats.vercel.app/api/wakatime?username=mehrdadmb2&layout=compact&theme=radical&hide_border=true&title_color=B44FE8&text_color=E0AAFF&bg_color=0d0d0d)](https://wakatime.com/@mehrdadmb2)
-->

</div>

---

## `~/contributions $ ./snake --watch`

<div align="center">

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/mehrdadmb2/mehrdadmb2/output/github-contribution-grid-snake-dark.svg"/>
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/mehrdadmb2/mehrdadmb2/output/github-contribution-grid-snake.svg"/>
  <img alt="GitHub contribution snake animation" src="https://raw.githubusercontent.com/mehrdadmb2/mehrdadmb2/output/github-contribution-grid-snake-dark.svg"/>
</picture>

</div>

---

## `~/achievements $ ls`

<div align="center">

<img src="https://github-profile-trophy.vercel.app/?username=mehrdadmb2&theme=radical&no-frame=true&no-bg=true&margin-w=6&margin-h=8&column=7" width="100%" alt="GitHub trophies"/>

</div>

---

## `~/blog $ cat latest-posts`

<!-- اگر از dev.to یا medium استفاده می‌کنید -->
<!-- 
<div align="center">

[![Dev.to](https://github-readme-medium.vercel.app/?username=mehrdad&limit=2)](https://dev.to/mehrdad)

</div>
-->

---

## `~/connect $ cat contact.md`

<div align="center">

I am open to collaborating on practical software, IoT, embedded systems and open-source projects.

<br/>

[![Discord](https://img.shields.io/badge/Discord-0mehrdad0-5865F2?style=for-the-badge&logo=discord&logoColor=white)](https://discord.gg/0mehrdad0)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Mehrdad_MB-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/mehrdad-mb-658520232)
[![Instagram](https://img.shields.io/badge/Instagram-@_._.m._.b-E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://www.instagram.com/_._.m._.b)
[![X](https://img.shields.io/badge/X-@__Mehrdad_-000000?style=for-the-badge&logo=x&logoColor=white)](https://x.com/__Mehrdad_)
[![Reddit](https://img.shields.io/badge/Reddit-Profile-FF4500?style=for-the-badge&logo=reddit&logoColor=white)](https://reddit.com/user/u/IllustriousEssay4051)

</div>

---

## `~/metrics $ uptime`

<div align="center">

![Profile Views](https://komarev.com/ghpvc/?username=mehrdadmb2&label=Profile%20Views&color=B44FE8&style=flat-square)
[![Followers](https://img.shields.io/github/followers/mehrdadmb2?style=flat-square&color=9D4EDD&label=Followers&logo=github&logoColor=white&labelColor=0d0d0d)](https://github.com/mehrdadmb2?tab=followers)
[![Stars](https://img.shields.io/github/stars/mehrdadmb2?affiliations=OWNER%2CCOLLABORATOR&style=flat-square&color=B44FE8&label=Total%20Stars&logo=github&logoColor=white&labelColor=0d0d0d)](https://github.com/mehrdadmb2)
[![Last Commit](https://img.shields.io/github/last-commit/mehrdadmb2/mehrdadmb2?style=flat-square&color=7B2FBE&label=Last%20Update&logo=github&logoColor=white&labelColor=0d0d0d)](https://github.com/mehrdadmb2/mehrdadmb2/commits/main)

</div>

<br/>

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=12,20,23&height=130&section=footer&animation=twinkling"/>

```text
[ system status ]  Online.
[ current task  ]  Learning, building and improving.
[ next command  ]  git push origin future
```

</div>
